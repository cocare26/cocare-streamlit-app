...
# الحصول على اسم الصفحة من الروابط
page = st.query_params.get("page", "")

# منطق التنقل بين الصفحات في Streamlit
if page == "create":
...
.signup{ display:block; text-align:center; font-size:13px; margin-top:15px; cursor:pointer; color:#222; text-decoration:none; }
...
<form class="form" id="loginForm" method="get" target="_top">
    ...
    <div class="forgot">
        <a href="/?page=forgot" target="_top" style="color:#555; text-decoration:none;">Forgot Password?</a>
    </div>
...
function login(){
    ...
    // إذا بدك يدخل على صفحة الموظف بمجرد ما يكبس (بدون شروط):
    // window.top.location.href = "/?page=employee";

    // المنطق الحالي: إذا كان الرقم 11 خانة يروح للموظف
    if(/^[0-9]{11}$/.test(v)){
        window.top.location.href = "/?page=employee";
    } 
        ...
        e.innerText = "يرجى إدخال رقم تعريف صحيح (11 رقم للموظفين)";
    }
}
</script>
</body>
