...
.circle{
 width:100px;height:100px;border-radius:50%;border:10px solid #2f80ed;
 display:flex;flex-direction:column;align-items:center;justify-content:center
}
.circle span{font-size:10px}.circle b{font-size:25px}

.head{display:flex;justify-content:space-between;align-items:start;margin-bottom:10px}
.title{font-size:19px;font-weight:800;line-height:1.2}
.section{font-size:15px;font-weight:800;margin:16px 0 8px}

.location{
 font-size:11px;border:none;background:white;border-radius:8px;padding:7px;
 box-shadow:0 2px 8px rgba(0,0,0,.10)
}
.map{height:98px;background:#e6ecf5;border-radius:13px;position:relative;overflow:hidden}
...
.alert{
 ...
 box-shadow:0 4px 14px rgba(0,0,0,.12);font-size:10.5px
}
.alert-head{color:white;padding:8px;font-weight:900;font-size:14px}
.red{background:#e94c4c}.yellow{background:#f2b72f}.blue{background:#2f80ed}
...
.chart-title{position:absolute;bottom:29px;left:0;width:100%;text-align:center;font-size:13px}
.chart-stars{position:absolute;bottom:8px;left:0;width:100%;text-align:center;color:#1267c9;font-size:18px}
...
.emp-name{font-size:18px;font-weight:900}
.emp-text{font-size:10.5px;line-height:1.25}

/* TODO */
.todo-title{text-align:center;font-size:20px;font-weight:900;margin-bottom:14px}
.task-input{
 width:100%;padding:12px;border:1px solid #d1d5db;border-radius:10px;
 font-size:14px;margin-bottom:10px
}
.todo-grid{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:10px}
.todo-grid select,.todo-grid input{
 width:100%;height:38px;border:1px solid #d1d5db;border-radius:8px;
 background:white;padding:6px;font-size:12px
}
.add-btn{
 width:100%;height:40px;background:#2f80ed;color:white;border:none;
 border-radius:9px;font-weight:900;cursor:pointer;margin-bottom:12px
}
.hide-all-btn{
 width:100%;height:36px;background:#eef2ff;color:#1d4ed8;border:none;
 border-radius:9px;font-weight:800;cursor:pointer;margin-bottom:12px
}
.empty{text-align:center;margin-top:70px;color:#6b7280;opacity:.6}
.clipboard{
 width:62px;height:62px;border-radius:14px;background:#eef6ff;margin:auto;
...
.task-title{font-weight:900;font-size:14px;margin-bottom:5px}
.badges{display:flex;flex-wrap:wrap;gap:5px;margin-bottom:8px}
.badge{padding:4px 7px;border-radius:7px;background:#eef2ff;font-size:11px}
.low{background:#dbeafe}.medium{background:#fff3bf}.high{background:#ffd6d6}
.done{background:#d1fae5}.pending{background:#fee2e2}.progress{background:#e0e7ff}
.task-actions{display:flex;gap:6px}
.task-actions button{
 border:none;border-radius:7px;padding:6px 8px;font-size:11px;font-weight:800;cursor:pointer
}
.delete-btn{background:#fee2e2;color:#b91c1c}
.hide-btn{background:#e0e7ff;color:#1d4ed8}
.hidden-task{display:none}

/* NAV */
.nav{
 position:absolute;bottom:0;left:18px;right:18px;height:70px;border-top:1px solid #e5e7eb;
 background:#fbfdff;display:flex;justify-content:space-around;align-items:center
}
.nav button{background:none;border:none;font-weight:900;font-size:14px;cursor:pointer;color:#111827}
...
<div id="todoPage" class="page">
    ...
    <button class="hide-all-btn" onclick="toggleTasks()">Hide / Show Tasks</button>
...
function addTask(){
    ...
    card.innerHTML = `
        ...
        <div class="task-actions">
            <button class="hide-btn" onclick="hideOneTask(this)">Hide</button>
...
function toggleTasks(){
    ...
    cards.forEach(card => {
        card.style.display = tasksHidden ? "none" : "block";
    });

    document.getElementById("emptyBox").style.display = tasksHidden ? "block" : "none";
}
